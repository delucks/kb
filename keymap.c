...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [0] = LAYOUT_ergodox_pretty(
    ...
    KC_ESCAPE,      KC_QUOTE,       LALT(KC_LSHIFT),KC_LGUI,        LGUI(LSFT(KC_H)),                                                                                                LGUI(LSFT(KC_L)),KC_RALT,        KC_LBRACKET,    KC_RBRACKET,    KC_RCTRL,
    ...
    switch (layer) {
      case 0:
        if(!disable_layer_color) {
          ...
          rgblight_sethsv_noeeprom(215,255,131);
        }
        break;
      case 1:
        if(!disable_layer_color) {
          rgblight_enable_noeeprom();
          rgblight_mode_noeeprom(1);
          rgblight_sethsv_noeeprom(196,255,255);
        }
        break;
      case 2:
        if(!disable_layer_color) {
          rgblight_enable_noeeprom();
          rgblight_mode_noeeprom(1);
          rgblight_sethsv_noeeprom(242,255,193);
        }
        break;
      default:
        if(!disable_layer_color) {
